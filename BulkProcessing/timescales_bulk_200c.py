...
def get_stored_filepaths_haloids(sim,z0haloid,return_z0_haloids=False):
    ...
    if return_z0_haloids:
        return filepaths, haloids, d['haloids'][sim].keys()
    else:
...
def bulk_processing(sim, savepath):
    filepaths, h1ids, z0haloids = get_stored_filepaths_haloids(sim, 1, return_z0_haloids=True)

    for j, (filepath, h1id) in enumerate(zip(filepaths,h1ids)):
        ...
        for z0haloid in z0haloids:
            if z0haloid == 1: continue;
            filepaths, haloids = get_stored_filepaths_haloids(sim, z0haloid)
            try:
                i = haloids[j]
            except IndexError:
                # this happens when a given halo doesn't have as much of a history (i.e. doesn't have haloids back to as far) as halo 1
                continue

            redshift = s.properties['z']
            print(f'{sim}: z=0 halo {z0haloid}, z={redshift:.2f} halo {i}')
            halo = h.load_copy(i)        
            ...
            rvir = properties['Rvir'] / s.properties['h'] * s.properties['a']
...
shortnames = ['h329.cosmo50PLK.3072g','h242.cosmo50PLK.3072g','h229.cosmo50PLK.3072g','h148.cosmo50PLK.3072g']
bases=['h329.cosmo50PLK.3072gst5HbwK1BH','h242.cosmo50PLK.3072gst5HbwK1BH','h229.cosmo50PLK.3072gst5HbwK1BH','h148.cosmo50PLK.3072g3HbwK1BH']
simshorts=['h329','h242','h229','h148']

for shortname, base, simshort in zip(shortnames, bases, simshorts):
    if not simshort=='h148': continue;
    savepath = f'../Data/timesteps_data/{simshort}.data' 
    ...
    bulk_processing(simshort, savepath)
